...
fn indexed_copy() -> (tempfile::TempDir, Store) {
    ...
    for entry in walkdir(&src) {
        ...
        if rel.starts_with(prefixgraph_core::STATE_DIR) {
            continue;
        }
...
fn pack_finds_relevant_code_with_explains_and_savings() {
    ...
    assert!(paths.iter().any(|p| p.contains("payments.py")), "items: {paths:?}");
...
fn planner_is_deterministic() {
    ...
    assert_eq!(ids_a, ids_b, "same query + snapshot must select identical, identically-ordered items");
}
...
fn budget_is_respected_with_signature_substitution() {
    ...
    assert!(pack.totals.selected_tokens <= 400, "totals: {:?}", pack.totals);
}
