...
use prefixgraph_core::model::{id_from_hex, id_hex, EdgeKind, NodeKind};
...
pub fn dispatch(store: &Store, name: &str, args: &Value) -> Result<Value> {
    ...
    match name {
        ...
        "build_context_pack" => {
            let query = s("query").ok_or_else(|| anyhow!("missing 'query'"))?;
            let stale = prefixgraph_indexer::dirty_files(store)?.len() as i64;
            let pack = build_pack(
                store,
                &PackRequest {
                    query,
...
pub fn search_code(store: &Store, query: &str, limit: usize) -> Result<Value> {
    let cands = prefixgraph_planner::gather(store, query, &[], limit.max(10))?;
    let hits: Vec<Value> = cands
        .iter()
        .take(limit)
        .map(|c| {
            let snippet: String = c.node.content.lines().take(4).collect::<Vec<_>>().join("\n");
            json!({
...
fn resolve_symbol_node(
    store: &Store,
    symbol: &str,
) -> Result<prefixgraph_core::model::Node> {
    store
...
pub fn explain_symbol(store: &Store, symbol: &str) -> Result<Value> {
    ...
    let rel = |edges: Vec<(EdgeKind, prefixgraph_core::model::NodeId, f64)>| -> Vec<Value> {
        edges
            .into_iter()
            .filter_map(|(k, id, _)| {
                ...
                Some(json!({"relation": k.label(), "path": n.path, "symbol": n.symbol,
...
pub fn changed_since(store: &Store) -> Result<Value> {
    let dirty = prefixgraph_indexer::dirty_files(store)?;
    ...
    Ok(json!({
        "changed_files": dirty,
        "last_index": report,
        "hint": if dirty.is_empty() { "index is fresh" } else { "run: prefixgraph index ." }
    }))
}
