...
use prefixgraph_core::model::NodeKind;
use prefixgraph_core::Store;
use prefixgraph_indexer::{index_repo, IndexOptions};
use std::fs;
use std::path::Path;
...
fn comment_only_change_preserves_sibling_chunks() {
    ...
    for id in &before {
        assert!(after.contains(id), "chunk identity must survive a comment-only change");
    }
    assert_eq!(report.chunks_staled, 0, "no chunk should be invalidated: {report:?}");
}
...
fn editing_one_function_invalidates_only_it() {
    ...
    let report = index_repo(&mut store, &IndexOptions { force: true }).unwrap();
    assert_eq!(report.chunks_staled, 1, "only process_payment should invalidate: {report:?}");
    assert_eq!(report.chunks_new, 1);
    assert!(report.chunks_preserved >= 1, "PaymentValidator must be preserved");
}
...
fn reindex_is_idempotent() {
    ...
    assert_eq!(second.chunks_new, 0, "second run must create nothing: {second:?}");
