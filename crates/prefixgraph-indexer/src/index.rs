...
use crate::walk;
use anyhow::Result;
use prefixgraph_core::hash::{content_hash, node_id, sha256_hex};
use prefixgraph_core::model::{EdgeKind, Node, NodeId, NodeKind};
use prefixgraph_core::store::FileRow;
use prefixgraph_core::{norm::pg_norm_1, tokens, Store};
...
pub fn index_repo(store: &mut Store, opts: &IndexOptions) -> Result<IndexReport> {
    ...
    for abs in &files {
        ...
        let Some(norm) = pg_norm_1(&raw) else {
