...
use packmind_core::pack::{ContextPack, PackItem};
...
pub fn render_plain(pack: &ContextPack) -> String {
    ...
    format!(
        "<!-- PackMind context pack {} ({} tokens, saved {:.1}%) -->\n{}{}",
...
pub fn render_openai(pack: &ContextPack) -> Value {
    ...
    h.update(packmind_core::ENVELOPE_VERSION.as_bytes());
    ...
    json!({
        ...
        "prompt_cache_key": format!("pm-{}", &key[..32])
    })
}
