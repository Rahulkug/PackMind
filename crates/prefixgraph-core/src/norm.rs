//! PG-NORM-1 content normalization.
//!
...
pub fn pg_norm_1(raw: &[u8]) -> Option<String> {
...
mod tests {
    ...
    fn idempotent() {
        ...
        for c in cases {
            let once = pg_norm_1(c.as_bytes()).unwrap();
            let twice = pg_norm_1(once.as_bytes()).unwrap();
    ...
    #[test]
    fn rules() {
        assert_eq!(pg_norm_1(b"a  \r\nb\r").unwrap(), "a\nb\n");
        assert_eq!(pg_norm_1("\u{feff}x".as_bytes()).unwrap(), "x\n");
        assert_eq!(pg_norm_1(b"x\n\n\n").unwrap(), "x\n");
        // interior whitespace and comments preserved
        assert_eq!(pg_norm_1(b"a\t b  # comment\n").unwrap(), "a\t b  # comment\n");
        // non-UTF-8 rejected
        assert!(pg_norm_1(&[0xff, 0xfe, 0x00]).is_none());
    }
}
