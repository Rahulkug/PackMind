...
//! `.prefixgraph/index.db` (WAL mode, single writer, many readers).
...
impl Store {
    ...
    pub fn open_existing(root: &Path) -> Result<Store> {
        ...
        if !db.exists() {
            return Err(anyhow!(
                "no PrefixGraph index found at {} — run: prefixgraph index .",
                root.display()
            ));
        }
        Store::open(root)
    ...
    pub fn stale_others(&self, path: &str, kind: NodeKind, keep: &HashSet<NodeId>) -> Result<i64> {
        let mut stmt = self.conn.prepare(
            "SELECT id FROM nodes WHERE path=?1 AND kind=?2 AND valid=1",
        )?;
    ...
    pub fn out_edges(&self, src: &NodeId, kinds: &[EdgeKind]) -> Result<Vec<(EdgeKind, NodeId, f64)>> {
        self.edges_query("src", "dst", src, kinds)
    }

    pub fn in_edges(&self, dst: &NodeId, kinds: &[EdgeKind]) -> Result<Vec<(EdgeKind, NodeId, f64)>> {
    ...
    pub fn hot_set(&self) -> Result<(i64, Vec<NodeId>)> {
        let version: i64 = self
            ...
            .unwrap_or(0);
        let mut stmt = self.conn.prepare(
            "SELECT node_id FROM hot_set WHERE version=?1 ORDER BY position",
        )?;
    ...
    pub fn symbol_entries(&self) -> Result<Vec<(NodeId, String, String, String)>> {
        ...
        let rows = stmt.query_map([], |r| {
            let id: Vec<u8> = r.get(0)?;
            Ok((id, r.get::<_, String>(1)?, r.get::<_, String>(2)?, r.get::<_, String>(3)?))
        })?;
        Ok(rows
            .filter_map(|r| r.ok())
