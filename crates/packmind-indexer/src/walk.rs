//! Repo file walker: respects .gitignore, .packmindignore, and legacy
//! .prefixgraphignore files, skips the
...
pub fn repo_files(root: &Path) -> Vec<PathBuf> {
    ...
    let walker = ignore::WalkBuilder::new(root)
        ...
        .git_global(true)
        .git_exclude(true)
        .add_custom_ignore_filename(".packmindignore")
    ...
    for entry in walker.flatten() {
        ...
        if path
            .components()
            .any(|c| c.as_os_str() == packmind_core::STATE_DIR)
        {
            continue;
        }
